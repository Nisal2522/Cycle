 ...
 * --------------------------------------------------
 * Socket.io chat: auth via JWT, join chat room, send message, typing, online.
 */
    ...
    socket.on("send_message", async (payload) => {
      ...
      chat.users.forEach((uid) => {
        ...
        if (uidStr !== senderIdStr) {
          chat.unreadCount.set(uidStr, (chat.unreadCount.get(uidStr) || 0) + 1);
        }
      });
